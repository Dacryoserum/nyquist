...
- Renders an interactive spectrogram (FFT) with playback and click-to-seek, and measures
  ...
  the track.
- Scans container tags for leftover lossy-encoder signatures (LAME, iTunes, ...).
...
decoding, `rustfft` for spectral analysis, `ebur128` for LUFS/LRA/true peak. Playback uses
the webview's native `<audio>` element via Tauri's `asset://` protocol, not a Rust audio
crate. macOS first, Windows and Linux planned after V1.0.
...
Ahead:
- Remaining UI polish (session history, drag & drop, explicit light/dark toggle).
